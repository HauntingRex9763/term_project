...
df <- read.csv("C:\\Users\\jacks\\OneDrive\\11UPEI\\Classes\\Stat 1910\\Term Project\\df3.csv") %>% 
  clean_names() %>%
...
# 1. Introduction 
# TODO Reffrence python script on github used to merge two csvs
...
To address this question, we used the^[KUKUROO3 _used car price dataset (competition format)_. Kaggle, n.d. Web. 19 Nov 2022. https://www.kaggle.com/datasets/kukuroo3/used-car-price-dataset-competition-format] data set from Kaggle that merged information from^[Aditya _100,000 UK Used Car Data set_. Kaggle, n.d. Web. 19 Nov 2022. https://www.kaggle.com/datasets/christophercorrea/dc-residential-properties?resource=download] which contains cars listed for sale from various used car listing websites.  The data was last updated four years ago. Each case in the data set is a property in DC. 

It was decided to have price of used cars for sale in the UK (in Euros) as our outcome variable. There is two explanatory variables in this analysis: the mileage of a car, and transmission type of a car, with three levels, "Automatic", "Semi-Auto", "NW", and "Manual". 
...
* The estimate for the slope for the price ($b_{prct}$ = -0.266) is the associated change in mileage depending on the transmission type fo the car. Based on this estimate, for every mile traveled, there was an associated decrease in price of on average 0.266 Euros
...
$$
\begin{aligned}
\text{Automatic schools (in red)}: \widehat {price} =& 34431.667 - 2.777 \cdot mileage\\
\text{Manual transmissions (in green)}: \widehat {price} =& 21152.328 - 2.777 \cdot mileage\\
\text{Semi-Auto transmissions (in blue)}: \widehat {price} =& 37704.386 - 2.777 \cdot mileage
\end{aligned} 
$$
...
immigration_stats <-read.csv("C:\\Users\\jacks\\OneDrive\\11UPEI\\Classes\\Stat 1910\\Term Project\\canadian_immegration_data.csv")